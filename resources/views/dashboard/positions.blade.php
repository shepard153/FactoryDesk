 ...
 @section('content')
        ...
        <table class="table table-striped table-hover responsive table align-middle">
            @if ($positions != null)
                ...
                @foreach($positions as $position)
                    <tr>
                        <form method="post" action="{{ url('editPositionAction') }}">
                            @csrf
                            <td style="width: 20%"><input class="form-control" type="text" name="position_name" value="{{ $position->position_name }}" id="{{ $position->positionID }}" disabled/></td>
                            <td style="width: 65%"><input class="form-control" type="text" name="zones_list" value="{{ $position->zones_list }}" id="list-{{ $position->positionID }}" disabled/></td>
                            <td style="width: 15%">
                                <input type="button" class="btn btn-success" name="edit" id="edit-{{ $position->positionID }}" data-id="{{ $position->positionID }}" value="Edytuj"/>
                                <button class="btn btn-success" name="save" id="save-{{ $position->positionID }}" value="{{ $position->positionID }}" style="display: none">Zapisz</button>
                                <input type="button" class="btn btn-danger" name="delete" data-bs-toggle="modal" data-bs-target="#modal" data-name="{{ $position->position_name }}" data-id="{{ $position->positionID }}" value="Usuń"/>
                            </td>
                        </form>

                            <!-- Okienko z potwierdzeniem -->
                            <div class="modal fade" id="modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <form method="post" action="{{ url('deletePositionAction') }}">
                                    @csrf
    ...
    <script>
        $("[name=delete]").click(function() {
            ...
            document.getElementById("confirmDelete").value = positionID;
        });
